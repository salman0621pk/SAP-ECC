...
*     call the method (web service call) you can use the pattern to generate the code if you wish
      call method proxy_test->ADD_CUSTOMER
        exporting
          input  = input
        importing
          output = output.
...
*     process the output
    CUSTOMER_RESPONSE = output-ADD_CUSTOMER_RESULT.
    successs = 'X'.
    catch cx_ai_system_fault into exc.
          msg = exc->GET_TEXT( ).
          CONCATENATE 'Error in Add Customer:' msg into msg.
